...
async def get_user_by_id(db: Any, user_id: str) -> dict[str, Any] | None:
...
async def update_user(db: Any, user_id: str, payload: UserUpdate) -> UserResponse:
    if not ObjectId.is_valid(user_id):
...
async def deactivate_user(db: Any, user_id: str) -> UserResponse:
    if not ObjectId.is_valid(user_id):
        raise UserServiceError("NOT_FOUND", "User not found", 404)
...
async def increment_failed_attempts(db: Any, user_id: ObjectId) -> int:
...
async def reset_failed_attempts(db: Any, user_id: ObjectId) -> None:
    await db.users.update_one(
        {"_id": user_id},
...
async def change_password(db: Any, user_id: ObjectId, new_hashed_password: str) -> None:
    await db.users.update_one(
        {"_id": user_id},
